...
- **2026-04-10**: **#944** Image Analysis auth readiness no longer collapses to native Read when a partial runtime-status dependency override omits `initializeAccounts`. CCS now treats account-registry warm-up as optional in that path, still reads token-backed auth status directly, and includes regression coverage for the missing-initializer case so Kimi-backed sessions do not fail on `deps.initializeAccounts is not a function`.
